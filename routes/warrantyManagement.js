...
router.post('/update/:id', upload.single('billPdf'), async (req, res) => {
    ...
    if (req.file) {
        updateData.billPdf = req.file.path;
    }
        ...
        pdf.create(pdfContent, pdfOptions).toFile(pdfPath, async (err, result) => {
            ...
            async function sendMail() {
                const info = await transporter.sendMail({
                    ...
                    to: `"Recipient" <${updateData.email}>`,
                    subject: "Warranty Verification Completed: Your Warranty is Attached",
                    text: `Dear Bitbox PC User,
                    Your bulk warranty verification details for batch number ${batchId} have been processed successfully. Please find the details attached.
                    ...
                    Warranty Expiry: ${formatDate(commonExpiryDate)} <br>
                    <h2>Coverage Details: ${updateData.warrantyType} </h2>  <br>
                    ...
                    attachments: [
                        {
                            filename: `bulk-warranty-verification-${batchId}.pdf`,
    ...
    try {
        ...
        <head>
            ...
            <style>
                ...
                .certificate-footer, .terms-conditions {
                    margin-top: 20px;
                    font-size: 14px;
                }
            </style>
        ...
        <body>
            <div class="certificate">
                ...
                <div class="certificate-content">
                    <p>This document certifies the warranty coverage for the product purchased from PATA Electric Company and serves as proof of your entitlement to warranty services.</p>
                    <table class="warranty-info">
                        <thead>
                            <tr>
                                <th>Product Model Number</th>
        ...
        pdfContent += `
                ...
                <div class="certificate-footer">
                    <p>PATA Electric Company warrants that the product mentioned above is free from defects in material and workmanship under normal use during the warranty period.</p>
                </div>
                <div class="terms-conditions">
                    <h3>Terms and Conditions:</h3>
                    <p>■ Warranty Period: The warranty period commences on the date of purchase and lasts for the duration specified on this certificate.</p>
...
router.post('/update-warranty/:warrantyId', upload.single('billPdf'), async (req, res) => {
    ...
    try {
        ...
        if (updatedWarrantyData.purchaseDate && updatedWarrantyData.duration) {
            const purchaseDateObj = new Date(updatedWarrantyData.purchaseDate);
            const expiryDateObj = new Date(purchaseDateObj);
            expiryDateObj.setFullYear(expiryDateObj.getFullYear() + parseInt(updatedWarrantyData.duration));
            expiryDateObj.setHours(0, 0, 0, 0); // Set to the start of the day

            updatedWarrantyData.expiryDate = expiryDateObj;
        }
        ...
        if (warranty.verify) {
            ...
            const warrantyImageSrc = getWarrantyImage(updatedWarrantyData.duration);
            const expiryDate = new Date(warranty.expiryDate).toDateString();
            console.log(warranty.serialNumber);
            const xxx = warranty.serialNumber;
    
            const serialDetails = await SerialNumber.findOne({ serialNumber: xxx });
        ...
        <div class="certificate">
            ...
            <div class="certificate-content">
                ...
                <div>
                    ...
                    <div>
                        <span>■ Date of Purchase:</span> ${formatDate(warranty.purchaseDate)}
                    ...
                    <div>
                        <span>■ Warranty Expiry:</span>${formatDate(warranty.expiryDate)}
                    </div>
